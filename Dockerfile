...
RUN ARCH=$(uname -m) && \
    if [ "$ARCH" = "x86_64" ]; then \
        curl -L https://micromamba.snakepit.net/api/micromamba/linux-64/latest | tar -xvzf - -C /opt/micromamba && \
        ln -s /opt/micromamba/bin/micromamba /usr/local/bin/micromamba; \
    elif [ "$ARCH" = "aarch64" ]; then \
        curl -L https://micromamba.snakepit.net/api/micromamba/linux-arm64/latest | tar -xvzf - -C /opt/micromamba && \
        ln -s /opt/micromamba/bin/micromamba /usr/local/bin/micromamba; \
    else \
